...
export default function RegisterPage() {
  ...
  const handleGoogleSignup = async () => {
    ...
    registerUser("alex.mercer@gmail.com", "Alex Mercer");
    router.push("/dashboard");
  };
    ...
    <div className="relative flex min-h-screen flex-col items-center justify-center p-4 bg-gradient-to-tr from-background via-primary/5 to-background">
      ...
      >
        ...
        <div className="flex flex-col items-center space-y-2 text-center">
          ...
          <p className="text-sm text-muted-foreground">Start generating premium itineraries with AI.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className={`w-full rounded-xl border bg-background/50 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-1 ${
                errors.name ? "border-destructive focus:ring-destructive" : "border-border/80 focus:border-primary focus:ring-primary"
              }`}
            />
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              {...register("email")}
              className={`w-full rounded-xl border bg-background/50 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-1 ${
                errors.email ? "border-destructive focus:ring-destructive" : "border-border/80 focus:border-primary focus:ring-primary"
              }`}
            />
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              {...register("password")}
              className={`w-full rounded-xl border bg-background/50 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-1 ${
                errors.password ? "border-destructive focus:ring-destructive" : "border-border/80 focus:border-primary focus:ring-primary"
              }`}
            />
        ...
        <div className="relative flex items-center justify-center my-4">
          ...
          <span className="absolute bg-card px-3 text-xs text-muted-foreground uppercase font-bold tracking-wider">Or</span>
        </div>

        {/* Google Signup Button */}
        <button
          onClick={handleGoogleSignup}
          className="w-full flex h-11 items-center justify-center space-x-2.5 rounded-xl border border-border/80 bg-card/65 text-sm font-semibold text-foreground hover:bg-muted transition-all"
        >
